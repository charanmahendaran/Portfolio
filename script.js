...
window.addEventListener('scroll', () => {
  ...
  const certOffset = certificates.offsetTop - navbar.offsetHeight;

  if (scrollPosition >= certOffset) {
    navbar.classList.add('bg-black/50', 'backdrop-blur-2xl', 'transition-all', 'duration-300');
    navbar.classList.remove('bg-transparent');
  } else {
    navbar.classList.remove('bg-black/50', 'backdrop-blur-sm');
    navbar.classList.add('bg-transparent','backdrop-blur-sm', 'transition-all', 'duration-300');
  }
});
